...
const TeacherForm = () => {
  ...
  useEffect(() => {
    const fetchAdminEmailDomain = async () => {
      try {
        const adminEmail = auth.currentUser?.email;
        if (!adminEmail) throw new Error("Admin not logged in.");
        const domain = adminEmail.split("@")[1];
        setEmailDomain(domain);
      } catch (error) {
        console.error("Error fetching admin email domain:", error.message);
        setError("Failed to fetch admin email domain.");
      }
    };
    fetchAdminEmailDomain();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!emailDomain) {
      setError("Email domain is not set.");
      return;
    }
